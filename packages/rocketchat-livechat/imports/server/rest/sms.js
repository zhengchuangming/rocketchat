...
RocketChat.API.v1.addRoute('livechat/sms-incoming/:service', {
	post() {
		...
		try {
			console.log("const message = SMSService.response.call(this, RocketChat.Livechat.sendMessage(sendMessage));");
			const message = SMSService.response.call(this, RocketChat.Livechat.sendMessage(sendMessage));

			Meteor.defer(() => {
				if (sms.extra) {
					console.log("const message = SMSService.response.call(this, RocketChat.Livechat.sendMessage(sendMessage))11111111111;");
