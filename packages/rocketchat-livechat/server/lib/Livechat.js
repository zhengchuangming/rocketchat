...
RocketChat.Livechat = {
	...
	sendMessage({ guest, message, roomInfo, agent }) {
		...
		console.log("Livechat.js/sendMessage({ guest, message, roomInfo, agent }) {");
		// return messages;
	...
	closeRoom({ user, visitor, room, comment }) {
		...
		console.log("Livechat.js/RocketChat.models.Subscriptions.hideByRoomIdAndUserId(room._id, room.servedBy._id);");
