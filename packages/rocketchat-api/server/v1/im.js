...
RocketChat.API.v1.addRoute(['dm.messages', 'im.messages'], { authRequired: true }, {
	get() {
		...
		const ourQuery = Object.assign({}, query, { rid: findResult.room._id });
		console.log("const messages = RocketChat.models.Messages.find(ourQuery, {");
...
RocketChat.API.v1.addRoute(['dm.messages.others', 'im.messages.others'], { authRequired: true }, {
	get() {
		...
		console.log("onst msgs = RocketChat.models.Messages.find(ourQuery, {");
		...
		console.log("otal: RocketChat.models.Messages.find(ourQuery).count(),");
		return RocketChat.API.v1.success({
			messages: msgs,
			offset,
			count: msgs.length,
