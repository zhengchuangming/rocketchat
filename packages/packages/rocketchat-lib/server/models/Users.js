...
class ModelUsers extends RocketChat.models._Base {
	...
	validUserInSite(siteId,usernameOrEmail){
		...
		console.log("validUserInSite(siteId,usernameOrEmail){",usernameOrEmail);
		const UserInfo = this.findOne(query);
		if(UserInfo && siteId == UserInfo.site_id)
			return true;
		else
			return false;
	}
	...
	findUsersByNameOrUsername(nameOrUsername, options) {
		...
		console.log("findUsersByNameOrUsername");
		return this.find(query, options);
	}
	...
	findByUsernameNameOrEmailAddress(usernameNameOrEmailAddress, options) {
		...
		console.log("findByUsernameNameOrEmailAddress");
		return this.find(query, options);
	}
	findByUsernameNameOrEmailAddressAndSiteId(siteId, usernameNameOrEmailAddress, options) {
		const query = {
			$or: [
				{ name: usernameNameOrEmailAddress },
				{ username: usernameNameOrEmailAddress },
	...
	findUsersByUsernames(usernames, options) {
		...
		console.log("findUsersByUsernames");
		return this.find(query, options);
	}
	...
	setSiteId(_id, site_url) {
		const update =
			{ $set: { site_id : site_url} };
	...
	setCustomFields(_id, fields) {
		console.log("setCustomFields(_id, fields) {");
		const values = {};
