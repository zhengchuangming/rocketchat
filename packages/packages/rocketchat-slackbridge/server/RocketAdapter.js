...
export default class RocketAdapter {
	...
	onMessage(rocketMessage) {
		try {
			console.log("RocketAdapter.js/onMessage(rocketMessage)");
	...
	processSendMessage(rocketMessage) {
		console.log("processSendMessage(rocketMessage) {");
	...
	getMessageAttachment(rocketMessage) {
		console.log("RocketAdapter.js/getMessageAttachment(rocketMessage)");
		if (!rocketMessage.file) {
			return;
		}
	...
	processMessageChanged(rocketMessage) {
		console.log("RocketAdapter.js/processMessageChanged(rocketMessage)");
		if (rocketMessage) {
	...
	createAndSaveMessage(rocketChannel, rocketUser, slackMessage, rocketMsgDataDefaults, isImporting) {
		console.log("createAndSaveMessage");
		if (slackMessage.type === 'message') {
			...
			console.log("slackMessage.bot_id && slackMessage.ts && !RocketChat.models.Messages.findOneBySlackBotIdAndSlackTs(slackMes");
			if (slackMessage.subtype === 'bot_message') {
				Meteor.setTimeout(() => {
