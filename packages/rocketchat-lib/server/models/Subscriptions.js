class ModelSubscriptions extends RocketChat.models._Base {
	...
	getLastSeen(options) {
		...
		options.sort = { ls: -1 };
		options.limit = 1;
		// console.log("const query = { ls: { $exists: 1 } };,**************");
	...
	setAsReadByRoomIdAndUserId(roomId, userId) {
		...
		console.log("setAsReadByRoomIdAndUserId(roomId, userId) {,**************");
		const update = {
			$set: {
				open: true,
				alert: false,
	...
	setAsUnreadByRoomIdAndUserId(roomId, userId, firstMessageUnreadTimestamp) {
		...
		console.log("sls: firstMessageUnreadTimestamp,**************");
		const update = {
			$set: {
				open: true,
				alert: true,
	...
	setCustomFieldsDirectMessagesByUserId(userId, fields) {
		console.log("setCustomFieldsDirectMessagesByUserId(userId, fields) {");
		const query = {
			'u._id': userId,
			t: 'd',
		};
