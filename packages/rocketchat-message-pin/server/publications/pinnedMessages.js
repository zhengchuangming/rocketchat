Meteor.publish('pinnedMessages', function(rid, limit = 50) {
	...
	console.log("const user = RocketChat.models.Users.findOneById(this.userId);");
	const user = RocketChat.models.Users.findOneById(this.userId);
	if (!user) {
		return this.ready();
	}
	console.log("const cursorHandle = RocketChat.models.Messages.findPinnedByRoom(rid, { sort: { ts: -1 }, limit }).observeChanges({");
